...
`.coa-table-scroll` is the horizontally scrollable, keyboard-focusable region used when the table's 680px minimum width does not fit the viewport. `.coa-table` supplies the Figma treatment: a black header with 16px padding and white 18px bold type, 64px body rows separated by black rules, white / gray-lightest (`#fafafa`) zebra stripes matching Figma node `5968:787`, right-aligned tabular-number cost columns, and a bold unruled total row. `.coa-table-heading-parenthetical` holds `(In-State)` and `(Out-of-State)` on dedicated second lines without changing their accessible header text. `.coa-footnotes` wraps a semantic list inside `.umd-text-rich-advanced`, retaining the rich-text disc markers; every footnote, including emphasis and links, is fixed at the requested 14px with the project's gradient-underline link treatment.
