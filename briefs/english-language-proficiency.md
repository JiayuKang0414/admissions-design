...
- Three text-only dark `umd-element-card-overlay` components for the accepted tests, using `umd-layout-grid-gap-three` and `umd-layout-grid-child-fill-height`. The cards form a three-column row at 768px and above and stack on mobile. Each headline links to the test provider; all descriptions, links, and scores remain visible.
